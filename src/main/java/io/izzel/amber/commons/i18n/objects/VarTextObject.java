...
public class VarTextObject extends LocaleObject implements MetaObject {
    private static final Pattern VAR_TEXT = Pattern.compile(
        "((&[0-9a-fA-Fk-oK-OrR])+)?\\{(-?\\d+)}(((?!((&[0-9a-fA-Fk-oK-OrR])+)?\\{(-?\\d+)}).)*)"
    );
    ...
    public static io.izzel.amber.commons.i18n.objects.VarTextObject of(String text) {
        return new io.izzel.amber.commons.i18n.objects.VarTextObject(text);
    }
    ...
    private Text build(Object... args) {
        ...
        for (val entry : tails.entrySet()) {
            ...
            builder
                .append(applyMeta(num, Arg.of(at(args, num)).toText(holder, args), args))
    ...
    public <T> T mapAs(TypeToken<T> typeToken, Object... args) {
        if (typeToken.equals(TypeToken.of(Text.class))) return (T) build(args); else return null;
    }
