...
class AmberLocaleMapper { // todo 这个硬编码真丑，得重写
    ...
    private LocaleObject parseTypedObject(ConfigurationNode node) {
        ...
        switch (type) {
            ...
            case "click":
                ...
                if (command != null) {
                    ...
                    if (command != null) {
                        ...
                        log.warn(
                            "Provide 'command' or 'suggest' in a click node, parsing node {}",
            ...
            default:
                log.warn(
                    "Unknown type {} while parsing node {}",
                    type,
                    Arrays.toString(node.getPath())
                );
                return SimpleStringObject.of("null");
    ...
    private void saveMap(ConfigurationNode node, Map<String, LocaleObject> map) {
        ...
        switch (node.getValueType()) {
            ...
            case MAP:
                if (
                    node.getChildrenMap().keySet().contains("type") ||
                    node.getChildrenMap().keySet().equals(META_OBJECT)
                ) {
                    map.put(key, parseObject(node));
                } else {
