...
class AmberLocaleLoader {
    ...
    CommentedConfigurationNode load(PluginContainer container, Locale info) {
        ...
        val asset = Sponge
            .getAssetManager()
            .getAsset(container, assetLocation)
            .orElseGet(
                () -> Sponge
                    .getAssetManager()
                    .getAsset(container, info.def())
                    .orElseThrow(
                        () -> new RuntimeException(
                            String.format("No default locale %s present.", info.def())
                        )
                    )
            );
    ...
    private void merge(CommentedConfigurationNode to, CommentedConfigurationNode from) {
        switch (from.getValueType()) {
            ...
            case MAP:
                if (
                    from.getChildrenMap().containsKey("type") ||
                    from.getChildrenMap().containsKey("meta")
                ) {
                    if (to.getValue() == null) {
                        to.setValue(from.getValue());
                        from.getComment().ifPresent(to::setComment);
