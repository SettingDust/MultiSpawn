...
class AmberLocaleProvider implements AmberLocale {
    ...
    public void to(MessageReceiver receiver, String path, Object... args) {
        ...
        if (opt.isPresent()) {
            ...
            receiver.sendMessage(
                Text.of(
                    TextColors.RED,
                    "Missing language node: " + path + " with args: " + Arrays.toString(args)
                )
            );
        }
    }
    ...
    public void reload() {
        ...
        val node = io.izzel.amber.commons.i18n.AmberLocaleLoader.load(container, info);
        locales = new io.izzel.amber.commons.i18n.AmberLocaleMapper(this).asMap(node);
    }

    @Override
    public Optional<Text> get(String path, Object... args) {
        return Optional
            .ofNullable(locales.get(path))
            .map(it -> it.mapAs(TypeToken.of(Text.class), args));
    }
